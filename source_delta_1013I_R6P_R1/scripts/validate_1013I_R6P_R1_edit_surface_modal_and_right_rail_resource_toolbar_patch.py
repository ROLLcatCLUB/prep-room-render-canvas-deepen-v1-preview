...
STAGE_ID = "1013I_R6P_R1_EDIT_SURFACE_MODAL_AND_RIGHT_RAIL_RESOURCE_TOOLBAR_PATCH"
...
BASE_DIR_NAME = "1013I_R6P_big_unit_section_view_edit_surface"
STAGE_DIR_NAME = "1013I_R6P_R1_edit_surface_modal_and_right_rail_resource_toolbar_patch"
BASE_HTML_NAME = "prep_room_render_canvas_deepen_v1_R6P_big_unit_section_view_edit_surface.html"
HTML_NAME = "prep_room_render_canvas_deepen_v1_R6P_R1_big_unit_section_edit_modal.html"
VALIDATOR_NAME = "validate_1013I_R6P_R1_edit_surface_modal_and_right_rail_resource_toolbar_patch.py"
...
def css_patch() -> str:
    ...
    /* 1013I_R6P_R1: edit is a modal; right rail returns to resource/tool area */
    ...
    .r6p-modal {
      ...
      overflow: auto;
      border: 1px solid rgba(43, 124, 106, .2);
      border-radius: 12px;
      background:
      ...
      box-shadow: 0 24px 56px rgba(20, 55, 44, .24);
...
def build_html(output_root: Path) -> str:
    ...
    source = source.replace("师维 · 备课室 | R6P 大单元章节查看编辑样张", "师维 · 备课室 | R6P_R1 大单元弹出编辑样张")
    source = source.replace("<!-- 1013I_R6P: section view/edit surface; static preview only. -->", "<!-- 1013I_R6P_R1: section edit moves to modal; right rail is resource/tool area. -->")
    source = source.replace("\n  </style>", css_patch() + "\n  </style>", 1)
    source = replace_right_rail(source)
    source = patch_script(source)
    source = source.replace("data-r6p-side-edit-surface=\"true\"", "data-r6p-side-edit-surface-removed=\"true\"")
    return source
...
def create_screenshots(stage_dir: Path, html_path: Path) -> dict[str, Any]:
    ...
    for viewport in [{"id": "desktop", "width": 1440, "height": 1100}, {"id": "mobile", "width": 390, "height": 1100}]:
        out = stage_dir / f"ui_smoke_screenshot_1013I_R6P_R1_{viewport['id']}.png"
        cmd = [
            str(browser),
            "--headless=new",
            "--disable-gpu",
...
def validate_html(html_text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "edit_surface_not_inline_body": "编辑章节 · " in html_text and "编辑章节 · " not in main,
...
def write_stage_files(output_root: Path, stage_dir: Path, result: dict[str, Any], smoke: dict[str, Any]) -> None:
    write_json(stage_dir / "right_rail_resource_toolbar_1013I_R6P_R1.json", {
        "stage": STAGE_ID,
    ...
    write_json(stage_dir / "section_edit_modal_surface_1013I_R6P_R1.json", {
        "stage": STAGE_ID,
        "edit_surface": "modal",
        "view_surface": "modal",
    ...
    write_json(stage_dir / "visual_smoke_1013I_R6P_R1.json", smoke)
    write_json(stage_dir / "1013I_R6P_R1_result.json", result)
    write_text(stage_dir / "1013I_R6P_R1_report.md", f"""# 1013I_R6P_R1 Edit Surface Modal And Right Rail Resource Toolbar Patch

FINAL_STATUS={FINAL_STATUS}
INHERITS_FROM={INHERITS_FROM}
NEXT_STAGE={NEXT_STAGE}

R6P_R1 corrects the R6P layout semantics:
- The right rail is restored as a resource library and tool area.
- Section view/edit opens in a modal instead of occupying the right rail.
- The main big-unit reading surface remains continuous.
...
- `{STAGE_DIR_NAME}/1013I_R6P_R1_result.json`
...
- `{STAGE_DIR_NAME}/{HTML_NAME}`
- `{STAGE_DIR_NAME}/right_rail_resource_toolbar_1013I_R6P_R1.json`
- `{STAGE_DIR_NAME}/section_edit_modal_surface_1013I_R6P_R1.json`
- `{STAGE_DIR_NAME}/1013I_R6P_R1_result.json`
- `{STAGE_DIR_NAME}/1013I_R6P_R1_report.md`
- `{STAGE_DIR_NAME}/ui_smoke_screenshot_1013I_R6P_R1_desktop.png`
- `{STAGE_DIR_NAME}/ui_smoke_screenshot_1013I_R6P_R1_mobile.png`
...
def main() -> None:
    ...
    result = {
        "stage": STAGE_ID,
        "status": FINAL_STATUS if not failed else "FAIL_1013I_R6P_R1_EDIT_SURFACE_MODAL_AND_RIGHT_RAIL_RESOURCE_TOOLBAR_PATCH",
        "final_status": FINAL_STATUS if not failed else "FAIL_1013I_R6P_R1_EDIT_SURFACE_MODAL_AND_RIGHT_RAIL_RESOURCE_TOOLBAR_PATCH",
        "inherits_from": INHERITS_FROM,
        "next_stage": NEXT_STAGE,
        "created_at": now(),
        **checks,
    ...
    source_delta = output_root / "source_delta_1013I_R6P_R1" / "scripts" / VALIDATOR_NAME
    source_delta.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(Path(__file__).resolve(), source_delta)
    if failed:
        raise SystemExit(json.dumps(result, ensure_ascii=False))
    print("ALL_1013I_R6P_R1_EDIT_SURFACE_MODAL_AND_RIGHT_RAIL_RESOURCE_TOOLBAR_PATCH_CHECKS_OK")
