...
STAGE_ID = "1013J_R1D_COURSEWARE_WORKSPACE_VISUAL_FIRST_COPY_POLISH"
...
BASE_DIR_NAME = "1013J_R1C_courseware_workspace_expanded_layout_polish"
STAGE_DIR_NAME = "1013J_R1D_courseware_workspace_visual_first_copy_polish"
BASE_HTML_NAME = "prep_room_render_canvas_deepen_v1_1013J_R1C_courseware_expanded_layout_polish.html"
HTML_NAME = "prep_room_render_canvas_deepen_v1_1013J_R1D_courseware_visual_first_copy_polish.html"
VALIDATOR_NAME = "validate_1013J_R1D_courseware_workspace_visual_first_copy_polish.py"
...
def patch_html(output_root: Path) -> str:
    ...
    html_text = html_text.replace(
        'data-1013j-r1c-expanded-polish="true">',
        'data-1013j-r1c-expanded-polish="true" data-1013j-r1d-visual-first-copy="true">',
        1,
    )
    html_text = html_text.replace("\n  </style>", css() + "\n  </style>", 1)
    ...
    function renderCoursewareExpandedWorkspace1013JR1(view) {
      ...
      return `
        <div class="courseware-r1c-shell" data-1013j-r1-expanded="true" aria-label="课件制作工作区">
          <header class="courseware-r1c-top courseware-r1d-top-minimal">
            <div>
...
def javascript_syntax_check(stage_dir: Path, html_text: str) -> dict[str, Any]:
    ...
    for index, script_text in enumerate(scripts):
        script_path = stage_dir / f"javascript_syntax_1013J_R1D_{index:02d}.js"
...
def screenshot(stage_dir: Path, html_path: Path) -> dict[str, Any]:
    ...
    for name, width, height, fragment in cases:
        out = stage_dir / f"ui_smoke_screenshot_1013J_R1D_{name}.png"
        cmd = [
            str(browser),
            "--headless=new",
            "--disable-gpu",
...
def validate_html(html_text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "current_screen_preview_created": "当前大屏预览" in html_text and "哪一组更安静？" in html_text,
        ...
        "xiaojiao_generated_courseware_status_visible": "8 屏大屏草稿" in html_text and "3 待补图" in html_text,
        ...
        "screen_title_restrained": "哪一组更安静？" in html_text and "哪一组颜色更安静？" not in html_text,
        "visual_placeholders_dominant": "min-height: 220px" in html_text and "热闹色彩图" in html_text and "安静色彩图" in html_text,
        ...
        "ai_generated_screen_text_present": "从颜色里找一找。" in html_text and "8 屏大屏草稿" in html_text,
        **boundary(),
    }
...
def write_docs(output_root: Path, stage_dir: Path, result: dict[str, Any]) -> None:
    write_text(stage_dir / "1013J_R1D_report.md", f"""# 1013J_R1D Courseware Workspace Visual First Copy Polish

FINAL_STATUS={result["final_status"]}
INHERITS_FROM={INHERITS_FROM}
NEXT_STAGE={NEXT_STAGE}

1013J_R1D keeps the R1C expanded workspace and removes over-explaining copy so the classroom screen becomes visual-first:
...
- `{STAGE_DIR_NAME}/1013J_R1D_result.json`
...
- `{STAGE_DIR_NAME}/{HTML_NAME}`
- `{STAGE_DIR_NAME}/courseware_workspace_state_1013J_R1D.json`
- `{STAGE_DIR_NAME}/courseware_screen_outline_1013J_R1D.json`
- `{STAGE_DIR_NAME}/courseware_placeholder_map_1013J_R1D.json`
- `{STAGE_DIR_NAME}/1013J_R1D_result.json`
- `{STAGE_DIR_NAME}/1013J_R1D_report.md`
- `{STAGE_DIR_NAME}/ui_smoke_screenshot_1013J_R1D_normal.png`
- `{STAGE_DIR_NAME}/ui_smoke_screenshot_1013J_R1D_expanded.png`
- `{STAGE_DIR_NAME}/ui_smoke_screenshot_1013J_R1D_mobile.png`
...
def main() -> None:
    ...
    base_result = output_root / BASE_DIR_NAME / "1013J_R1C_result.json"
    ...
    result = {
        "stage": STAGE_ID,
        "status": FINAL_STATUS if not failed else "FAIL_1013J_R1D_COURSEWARE_WORKSPACE_VISUAL_FIRST_COPY_POLISH",
        "final_status": FINAL_STATUS if not failed else "FAIL_1013J_R1D_COURSEWARE_WORKSPACE_VISUAL_FIRST_COPY_POLISH",
        "inherits_from": INHERITS_FROM,
        "next_stage": NEXT_STAGE,
        "created_at": now(),
        **checks,
    ...
    write_json(stage_dir / "1013J_R1D_result.json", result)
    write_json(stage_dir / "courseware_screen_outline_1013J_R1D.json", {"stage": STAGE_ID, "screen_count": len(SCREENS), "screens": SCREENS})
    write_json(stage_dir / "courseware_placeholder_map_1013J_R1D.json", {
        "stage": STAGE_ID,
        "placeholders": [{"screen_id": screen["id"], "placeholder": screen["placeholder"], "status": screen["status"]} for screen in SCREENS],
        **boundary(),
    })
    write_json(stage_dir / "courseware_workspace_state_1013J_R1D.json", {
        "stage": STAGE_ID,
    ...
    source_delta = output_root / "source_delta_1013J_R1D" / "scripts" / VALIDATOR_NAME
    source_delta.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(Path(__file__).resolve(), source_delta)
    if failed:
        raise SystemExit(json.dumps(result, ensure_ascii=False))
    print("ALL_1013J_R1D_COURSEWARE_WORKSPACE_VISUAL_FIRST_COPY_POLISH_CHECKS_OK")
