...
STAGE_ID = "1013J_R1E_COURSEWARE_SCREEN_RATIO_AND_VISUAL_DOMINANCE_PATCH"
...
BASE_DIR_NAME = "1013J_R1D_courseware_workspace_visual_first_copy_polish"
STAGE_DIR_NAME = "1013J_R1E_courseware_screen_ratio_and_visual_dominance_patch"
BASE_HTML_NAME = "prep_room_render_canvas_deepen_v1_1013J_R1D_courseware_visual_first_copy_polish.html"
HTML_NAME = "prep_room_render_canvas_deepen_v1_1013J_R1E_courseware_screen_ratio_and_visual_dominance.html"
VALIDATOR_NAME = "validate_1013J_R1E_courseware_screen_ratio_and_visual_dominance_patch.py"
...
def css() -> str:
    ...
    [data-1013j-r1c-expanded-polish="true"] .courseware-r1d-top-minimal {
      align-items: center;
      padding-bottom: 12px;
    }
    ...
    [data-1013j-r1c-expanded-polish="true"] .courseware-r1d-quiet-status {
      display: inline-flex;
      flex-wrap: wrap;
      gap: 8px;
      align-items: center;
      margin-top: 4px;
      color: rgba(29, 39, 35, 0.58);
      font-size: 12px;
      font-weight: 750;
    }
    ...
    [data-1013j-r1c-expanded-polish="true"] .courseware-r1c-screen-frame {
      ...
      border-radius: 18px;
      padding: 18px;
      background: linear-gradient(145deg, rgba(18, 77, 65, 0.12), rgba(18, 77, 65, 0.035));
    ...
    [data-1013j-r1c-expanded-polish="true"] .courseware-r1c-screen .courseware-placeholder-grid {
      min-height: 58%;
      align-self: stretch;
    }

    [data-1013j-r1c-expanded-polish="true"] .courseware-r1c-screen .courseware-placeholder {
      min-height: 220px;
      font-size: 12px;
      color: rgba(32, 75, 65, 0.5);
      background:
        linear-gradient(135deg, rgba(239, 248, 243, 0.48), rgba(255, 255, 250, 0.78));
    }

    [data-1013j-r1c-expanded-polish="true"] .courseware-r1d-light-interaction {
      padding: 7px 9px;
      border-radius: 10px;
      border: 1px solid rgba(44, 110, 163, .16);
      background: rgba(235, 244, 250, .36);
      color: rgba(32, 75, 65, .58);
      font-size: 12px;
    }
    ...
    [data-1013j-r1c-expanded-polish="true"] .courseware-r1d-side-compact .courseware-side-block p {
      color: rgba(29, 39, 35, 0.58);
      font-size: 12px;
      line-height: 1.45;
    }

    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-shell {
      width: min(1520px, calc(100vw - 36px));
      min-height: calc(100vh - 228px);
      margin: 14px auto 44px;
      padding: 16px;
      border-radius: 20px;
      border: 1px solid rgba(38, 126, 109, 0.18);
      background: linear-gradient(136deg, rgba(255, 253, 247, 0.98), rgba(238, 248, 240, 0.96));
      box-shadow: 0 24px 60px rgba(16, 61, 53, 0.12);
    }

    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-workbench {
      display: grid;
      grid-template-columns: 176px minmax(820px, 1fr) 172px;
      gap: 14px;
      align-items: start;
    }

    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-left,
    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-right {
      border-radius: 16px;
      border: 1px solid rgba(38, 126, 109, 0.14);
    ...
    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-title {
      color: var(--green);
      font-size: 13px;
      font-weight: 900;
      margin-bottom: 8px;
    ...
    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-status {
      display: grid;
      gap: 7px;
      color: rgba(29, 39, 35, 0.58);
      font-size: 12px;
    ...
    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-screen-list {
      display: grid;
      gap: 6px;
      margin-top: 12px;
    }

    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-screen-list .node-action {
      justify-content: flex-start;
      width: 100%;
      padding: 7px 9px;
      border-radius: 11px;
    ...
    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-main {
      display: grid;
      gap: 8px;
      align-content: start;
    }

    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-toolbar {
      display: flex;
      justify-content: space-between;
      align-items: center;
      gap: 10px;
    ...
    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-tools,
    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-ratio {
      display: inline-flex;
      align-items: center;
      gap: 6px;
    }

    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-icon {
      width: 30px;
      height: 30px;
      border-radius: 999px;
      border: 1px solid rgba(38, 126, 109, 0.2);
    ...
    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-icon.primary {
      background: var(--green);
      color: #fff;
    }

    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-segment {
      display: inline-flex;
      padding: 3px;
      border-radius: 999px;
      border: 1px solid rgba(38, 126, 109, 0.18);
    ...
    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-segment span {
      padding: 4px 10px;
      border-radius: 999px;
    ...
    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-segment span.active {
      background: var(--green);
      color: #fff;
    }

    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-screen-frame {
      width: 100%;
      aspect-ratio: 16 / 9;
      border-radius: 18px;
      padding: 16px;
      background: linear-gradient(145deg, rgba(18, 77, 65, 0.11), rgba(18, 77, 65, 0.03));
      box-shadow: inset 0 0 0 1px rgba(38, 126, 109, 0.14), 0 20px 48px rgba(16, 61, 53, 0.1);
    }

    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-screen {
      height: 100%;
      border-radius: 16px;
      border: 1px solid rgba(38, 126, 109, 0.18);
      background:
        radial-gradient(circle at 30px 30px, rgba(43, 124, 106, 0.035) 1px, transparent 1.5px),
        linear-gradient(135deg, rgba(255, 255, 251, 0.99), rgba(248, 246, 238, 0.98));
    ...
    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-question {
      color: rgba(18, 34, 29, 0.82);
    ...
    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-image-grid {
      display: grid;
      grid-template-columns: 1fr 1fr;
      gap: 16px;
      min-height: 0;
    ...
    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-image-slot {
      display: grid;
      place-items: center;
      border-radius: 14px;
      border: 1px dashed rgba(43, 124, 106, 0.26);
      background:
        linear-gradient(135deg, rgba(239, 248, 243, 0.48), rgba(255, 255, 250, 0.82));
      color: rgba(32, 75, 65, 0.44);
      font-size: 12px;
      text-align: center;
    }

    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-bottom-tools {
      display: flex;
      justify-content: space-between;
      align-items: center;
      min-height: 26px;
      color: rgba(32, 75, 65, 0.48);
      font-size: 12px;
    }

    [data-1013j-r1e-screen-ratio="true"] .courseware-r1e-right .node-action {
      width: 100%;
      justify-content: center;
      margin-bottom: 8px;
    }
...
def patch_html(output_root: Path) -> str:
    ...
    html_text = html_text.replace(
        "师维 · 备课室 | 1013J_R1D 课件视觉优先文案打磨",
        "师维 · 备课室 | 1013J_R1E 屏幕比例与图片主导",
        1,
    )
    html_text = html_text.replace(
        'data-1013j-r1d-visual-first-copy="true">',
        'data-1013j-r1d-visual-first-copy="true" data-1013j-r1e-screen-ratio="true">',
        1,
    )
    html_text = html_text.replace("\n  </style>", css() + "\n  </style>", 1)
    html_text = html_text.replace("当前屏预览 · 不是空白白板", "当前大屏预览")
    html_text = html_text.replace("白板只作为这一屏里的圈画工具，不是整个课件制作区的默认形态。", "圈画 / 标注")
    html_text = html_text.replace("可圈画：让学生指出“安静”的颜色。", "圈画 / 标注")
    html_text = html_text.replace("上传、搜索、白板库和 PPT 导出暂不实现。", "补图片、改问题、加入互动。")
    html_text = html_text.replace("后续能力", "操作")
    html_text = html_text.replace("哪一组颜色更安静？", "哪一组更安静？")
    html_text = html_text.replace("你从哪些颜色看出来？", "从颜色里找一找。")
    html_text = html_text.replace("已根据备课内容生成 8 屏大屏草稿", "8 屏大屏草稿")
    html_text = html_text.replace("3 处需要补图片，1 处适合课堂圈画。当前屏对应备课段“比较变化”。", "3 待补图 · 1 可圈画")
    html_text = html_text.replace("当前大屏预览", "")
    html_text = html_text.replace("03 · 比较变化", "")
    html_text = html_text.replace("屏幕序列", "")
    html_text = html_text.replace("8 屏大屏草稿", "8 屏")
    html_text = html_text.replace("courseware-r1d-top-minimal", "courseware-r1d-top-legacy-hidden")
    html_text = html_text.replace("<strong>当前备课段：</strong>", "")
    html_text = html_text.replace("<strong>对应课件屏：</strong>", "")
    html_text = html_text.replace("<strong>本屏作用：</strong>", "")
    html_text = html_text.replace("当前备课段：", "")
    override = r'''
    function renderCoursewareExpandedWorkspace1013JR1(view) {
      const current = coursewareScreens1013JR1[2];
      return `
        <div class="courseware-r1e-shell" data-1013j-r1-expanded="true" aria-label="课件制作工作区">
          <div class="courseware-r1e-workbench">
            ...
            <main class="courseware-r1e-main" aria-label="课堂大屏画面">
              <div class="courseware-r1e-toolbar" aria-label="大屏工具条">
                <div class="courseware-r1e-tools">
                  <button class="courseware-r1e-icon primary" type="button" title="进入大屏预览" data-pending="静态样张：进入大屏预览后续再接。">▶</button>
                  <button class="courseware-r1e-icon" type="button" title="补图片" data-pending="后续接素材投送。">图</button>
                  <button class="courseware-r1e-icon" type="button" title="圈画" data-pending="后续接局部互动块。">圈</button>
                  <button class="courseware-r1e-icon" type="button" title="改文字" data-pending="后续接文字编辑。">字</button>
                </div>
            ...
            <aside class="courseware-r1e-right" aria-label="课件操作">
              ...
              <button class="node-action primary" type="button" data-pending="静态样张：进入大屏预览后续再接。">大屏预览</button>
...
def javascript_syntax_check(stage_dir: Path, html_text: str) -> dict[str, Any]:
    ...
    for index, script_text in enumerate(scripts):
        script_path = stage_dir / f"javascript_syntax_1013J_R1E_{index:02d}.js"
...
def screenshot(stage_dir: Path, html_path: Path) -> dict[str, Any]:
    ...
    for name, width, height, fragment in cases:
        out = stage_dir / f"ui_smoke_screenshot_1013J_R1E_{name}.png"
        cmd = [
            str(browser),
            "--headless=new",
            "--disable-gpu",
...
def validate_html(html_text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "expanded_workspace_visually_dominant": "courseware-r1e-shell" in html_text and "courseware-r1e-workbench" in html_text,
        "current_screen_16_9_preview": "courseware-r1e-screen-frame" in html_text and "aspect-ratio: 16 / 9" in html_text,
        "screen_ratio_toggle_present": "16:9" in html_text and "4:3" in html_text and "courseware-r1e-segment" in html_text,
        "ratio_toggle_placed_above_screen": "courseware-r1e-toolbar" in html_text and "courseware-r1e-ratio" in html_text,
        "large_blank_grid_reduced": "width: min(1520px, calc(100vw - 36px))" in html_text and "min-height: calc(100vh - 228px)" in html_text,
        "top_explanation_bar_removed": "courseware-r1d-top-minimal" not in html_text and "8 屏大屏草稿" not in html_text,
        "main_screen_moved_to_top": "courseware-r1e-toolbar" in html_text and "courseware-r1e-screen-frame" in html_text,
        "current_screen_labels_removed": "当前大屏预览" not in html_text and "03 · 比较变化" not in html_text,
        "screen_sequence_label_removed": "屏幕序列" not in html_text,
        "current_screen_preview_created": "哪一组更安静？" in html_text and "courseware-r1e-image-grid" in html_text,
        "screen_outline_present": "courseware-r1e-screen-list" in html_text and "03" in html_text,
        "material_placeholders_visible": "热闹色彩图" in html_text and "安静色彩图" in html_text and "待补图" in html_text,
        "teacher_visible_engineering_caveats_absent": all(term not in html_text for term in ["暂不实现", "不是整个课件制作区", "不是空白白板", "后续能力"]),
        "whiteboard_as_local_interaction_tool": "data-whiteboard-as-block=\"true\"" in html_text and "圈画 / 标注" in html_text,
        "whiteboard_as_block_not_workspace": "data-whiteboard-as-block=\"true\"" in html_text and "圈画 / 标注" in html_text,
        "xiaojiao_generated_courseware_status_visible": "课件草稿" in html_text and "3 待补图" in html_text,
        "lesson_to_screen_mapping_visible": "比较变化" in html_text and "03 比较两组颜色" in html_text,
        "lesson_to_screen_mapping_lightweight": "比较变化" in html_text and "03 比较两组颜色" in html_text and "当前备课段：" not in html_text,
        "intro_explanation_removed": "已根据备课内容生成 8 屏大屏草稿" not in html_text and "当前屏对应备课段" not in html_text,
        "screen_title_restrained": "courseware-r1e-question" in html_text and "font-size: clamp(16px, 1.45vw, 24px)" in html_text,
        "visual_placeholders_dominant": "courseware-r1e-image-grid" in html_text and "courseware-r1e-image-slot" in html_text and "grid-template-rows: auto 1fr auto" in html_text,
        "bottom_interaction_copy_minimal": "圈画 / 标注" in html_text and "让学生指出" not in html_text,
        "xiaojiao_screen_suggestion_present": "先看图，再圈出颜色依据" in html_text,
        ...
        "main_lesson_notebook_reference_kept": "比较变化" in html_text,
        "courseware_screen_directory_visible": "courseware-r1e-left" in html_text and "课件草稿" in html_text,
        ...
        "ai_generated_screen_text_present": "从颜色里找一找。" in html_text and "8 屏" in html_text,
        **boundary(),
    }
...
def write_docs(output_root: Path, stage_dir: Path, result: dict[str, Any]) -> None:
    write_text(stage_dir / "1013J_R1E_report.md", f"""# 1013J_R1E Courseware Screen Ratio And Visual Dominance Patch

FINAL_STATUS={result["final_status"]}
INHERITS_FROM={INHERITS_FROM}
NEXT_STAGE={NEXT_STAGE}

1013J_R1E keeps the R1D visual-first workspace and adds screen-ratio controls while making images the dominant classroom material:
...
- `{STAGE_DIR_NAME}/1013J_R1E_result.json`
...
- `{STAGE_DIR_NAME}/{HTML_NAME}`
- `{STAGE_DIR_NAME}/courseware_workspace_state_1013J_R1E.json`
- `{STAGE_DIR_NAME}/courseware_screen_outline_1013J_R1E.json`
- `{STAGE_DIR_NAME}/courseware_placeholder_map_1013J_R1E.json`
- `{STAGE_DIR_NAME}/1013J_R1E_result.json`
- `{STAGE_DIR_NAME}/1013J_R1E_report.md`
- `{STAGE_DIR_NAME}/ui_smoke_screenshot_1013J_R1E_normal.png`
- `{STAGE_DIR_NAME}/ui_smoke_screenshot_1013J_R1E_expanded.png`
- `{STAGE_DIR_NAME}/ui_smoke_screenshot_1013J_R1E_mobile.png`
...
def main() -> None:
    ...
    base_result = output_root / BASE_DIR_NAME / "1013J_R1D_result.json"
    ...
    result = {
        "stage": STAGE_ID,
        "status": FINAL_STATUS if not failed else "FAIL_1013J_R1E_COURSEWARE_SCREEN_RATIO_AND_VISUAL_DOMINANCE_PATCH",
        "final_status": FINAL_STATUS if not failed else "FAIL_1013J_R1E_COURSEWARE_SCREEN_RATIO_AND_VISUAL_DOMINANCE_PATCH",
        "inherits_from": INHERITS_FROM,
        "next_stage": NEXT_STAGE,
        "created_at": now(),
        **checks,
    ...
    write_json(stage_dir / "1013J_R1E_result.json", result)
    write_json(stage_dir / "courseware_screen_outline_1013J_R1E.json", {"stage": STAGE_ID, "screen_count": len(SCREENS), "screens": SCREENS})
    write_json(stage_dir / "courseware_placeholder_map_1013J_R1E.json", {
        "stage": STAGE_ID,
        "placeholders": [{"screen_id": screen["id"], "placeholder": screen["placeholder"], "status": screen["status"]} for screen in SCREENS],
        **boundary(),
    })
    write_json(stage_dir / "courseware_workspace_state_1013J_R1E.json", {
        "stage": STAGE_ID,
    ...
    source_delta = output_root / "source_delta_1013J_R1E" / "scripts" / VALIDATOR_NAME
    source_delta.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(Path(__file__).resolve(), source_delta)
    if failed:
        raise SystemExit(json.dumps(result, ensure_ascii=False))
    print("ALL_1013J_R1E_COURSEWARE_SCREEN_RATIO_AND_VISUAL_DOMINANCE_PATCH_CHECKS_OK")
