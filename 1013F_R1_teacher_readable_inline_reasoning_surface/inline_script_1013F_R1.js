    ...
    function readableStepPlan(step) {
      ...
      return plans[step.id] || {
        ...
        hover: "这一段暂时没有可用的小备旁注。",
        note: [["小备旁注", "这一段暂时没有可用的小备旁注。"]]
      };
    }
    ...
    function renderSelectedParagraphNote(view, plan, paragraphId) {
      ...
      const items = (plan.note || []).slice(0, 4);
      return `
        <div class="nb-inline-note-panel" data-inline-note>
          <div><strong>小备旁注</strong>这条说明只跟随当前段落，点空白处可以收起。</div>
          <div class="nb-note-grid">
            ${items.map(([label, text]) => `<div class="nb-note-item"><strong>${html(label)}：</strong>${html(text)}</div>`).join("")}
          </div>
        </div>
      `;
    }
      ...
      return `
        <section class="nb-doc-section" id="nb-section-teaching-process">
          <div class="nb-doc-section-head">
            ...
            <span class="quiet-tag">轻点段落看小备旁注</span>
          </div>
    ...
    function renderReasoningBinding1013F(view) {
      ...
      return `
        <section class="nb-binding-card" id="nb-1013f-binding" aria-label="小备课堂推演候选">
          ...
          <div class="nb-right-note">这条提示先帮助老师把本课读成一条课堂推进线；具体怎么上，已经放回下面的教学过程正文。轻点段落，可以看到小备为什么这样安排。</div>
        </section>
      `;
    }
    ...
    function showHoverNote(text, event) {
      const note = byId("nbHoverNote");
      if (!note || !text) return;
      note.innerHTML = `<strong>小备判断</strong>${html(text)}`;
      const gap = 16;
      const width = 320;
