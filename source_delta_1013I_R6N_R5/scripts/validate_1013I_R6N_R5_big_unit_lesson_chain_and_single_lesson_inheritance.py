...
STAGE_ID = "1013I_R6N_R5_BIG_UNIT_LESSON_CHAIN_AND_SINGLE_LESSON_INHERITANCE"
...
NEXT_STAGE = "USER_REVIEW_BIG_UNIT_LESSON_CHAIN"
STAGE_DIR_NAME = "1013I_R6N_R5_big_unit_lesson_chain_and_single_lesson_inheritance"
...
HTML_NAME = "prep_room_render_canvas_deepen_v1_R6N_R5_big_unit_lesson_chain_and_single_lesson_inheritance.html"
VALIDATOR_NAME = "validate_1013I_R6N_R5_big_unit_lesson_chain_and_single_lesson_inheritance.py"
...
def r6n_r4_css() -> str:
    ...
    /* 1013I_R6N_R5: lesson chain and single-lesson inheritance */
    ...
    function renderBigUnitPrepSurface(view) {
      return `
        <section class="nb-workspace" aria-label="大单元设计">
          ...
          <article class="nb-unit-reading-doc" data-r6n-r5-lesson-chain-page="true">
            ...
            <section class="nb-unit-reading-section">
              ...
              <div>
                ...
                <p class="nb-single-inheritance-note">进入单课备课时，顶部只保留轻继承信息：单元“多变的色彩”，本课位置“比较方法”。悬浮或右侧折叠中显示上一课、本课、下一课的承接关系。</p>
              </div>
            </section>
            ...
            <section class="nb-unit-reading-section">
              <span class="nb-section-no">10</span>
              <div>
...
def build_html(output_root: Path) -> str:
    base = output_root / R6N_R4_DIR_NAME / "prep_room_render_canvas_deepen_v1_R6N_R4_big_unit_design_content_rewrite.html"
    source = base.read_text(encoding="utf-8")
    source = source.replace("师维 · 备课室 | R6N_R4 大单元内容重写", "师维 · 备课室 | R6N_R5 课时任务链")
    source = source.replace('data-r6n-r4-research-content-page="true"', 'data-r6n-r5-lesson-chain-page="true"')
    ...
    source = source.replace(
        ...
        "<!-- 1013I_R6N_R5: lesson chain and single-lesson inheritance; preview only, no runtime/provider/formal apply. -->",
    )
    return source
...
def create_screenshots(stage_dir: Path, html_path: Path) -> dict[str, Any]:
    ...
    for viewport in [{"id": "desktop", "width": 1440, "height": 1100}, {"id": "mobile", "width": 390, "height": 1100}]:
        out = stage_dir / f"ui_smoke_screenshot_1013I_R6N_R5_{viewport['id']}.png"
        cmd = [
            str(browser),
            "--headless=new",
            "--disable-gpu",
...
def build_result(output_root: Path, html_path: Path, visual_smoke: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        ...
        "material_request_actions_lightweight": all(text in html_text for text in ["上传教材目录", "上传单元页 / 教参截图", "补充本单元课时安排", "粘贴教参单元建议", "补充已有单元安排", "先按 3 课时临时预览"]) and "补充任意一种资料即可" in body,
        "main_surface_no_engineering_terms": hits == [],
        "main_surface_raw_engineering_field_hits": hits,
        "layout_style_inherits_r6n_r4": 'data-r6n-r5-lesson-chain-page="true"' in html_text and "nb-light-tag" in html_text and "nb-section-help" in html_text,
    ...
    result["final_status"] = FINAL_STATUS if not failures else "FAIL_1013I_R6N_R5_BIG_UNIT_LESSON_CHAIN_AND_SINGLE_LESSON_INHERITANCE"
    return result
...
LAYOUT_STYLE_INHERITS_R6N_R4={str(result["layout_style_inherits_r6n_r4"]).lower()}
...
{STAGE_DIR_NAME}/1013I_R6N_R5_result.json
{STAGE_DIR_NAME}/1013I_R6N_R5_report.md
scripts/{VALIDATOR_NAME}
source_delta_1013I_R6N_R5/scripts/{VALIDATOR_NAME}
{RESEARCH_NAME}
```
""")
    write_text(stage_dir / "1013I_R6N_R5_report.md", f"""# 1013I_R6N_R5 Report

This patch adds a lesson-task chain and single-lesson inheritance layer on top of R6N_R4.
...
def validate_result(result: dict[str, Any]) -> None:
    if result.get("failed_checks"):
        raise SystemExit("R6N_R5 validation failed: " + ", ".join(result["failed_checks"]))
...
def main() -> None:
    ...
    write_json(stage_dir / "visual_smoke_1013I_R6N_R5.json", visual_smoke)
    result = build_result(output_root, html_path, visual_smoke)
    result["secret_scan_hits"] = scan_secrets([html_path])
    write_json(stage_dir / "1013I_R6N_R5_result.json", result)
    write_json(stage_dir / "lesson_chain_manifest_1013I_R6N_R5.json", {"stage": STAGE_ID, "inherits_from": INHERITS_FROM, "research_source": RESEARCH_NAME, "boundary": boundary()})
    write_docs(output_root, stage_dir, result)
    source_delta = output_root / "source_delta_1013I_R6N_R5" / "scripts"
    ...
    print("ALL_1013I_R6N_R5_BIG_UNIT_LESSON_CHAIN_CHECKS_OK")
