      ...
      return `
        <section class="nb-doc-section ${focused ? "section-editing" : ""}" id="nb-section-${html(section.id)}">
          ...
          <ol class="nb-doc-body nb-numbered-body">
            ${(section.body || []).map((line) => `<li class="nb-numbered-item">${html(line)}</li>`).join("")}
          </ol>
    ...
    function handlePrepEditTarget(value) {
      ...
      const isSameProcess = kind === "process" && prepNotebookMode(prepView) === "edit" && target.stepId === id;
      const isSameSection = kind !== "process" && prepNotebookMode(prepView) === "edit" && target.sectionId === id;
