    ...
    window.PREP_ROOM_RENDER_VIEW_MODEL = {
      ...
      negotiation: {
        ...
        understanding: [
          ...
          "小备判断：先处理今日和明日课前包缺口，周六调休课由小教协同保持预演。",
        ...
        },
        {
          ...
          current_lesson: {
            ...
            reasoning_binding_1013F: {
              ...
              candidate_error_message: "小备这次没有整理出可用候选，建议重新说一下你想调整的地方。"
            },
            status_cards: [
              ["正式课位", "8节", "done"],
              ["查看状态", "可阅读", "done"],
    ...
    function renderProcessStep(view, step, index) {
      ...
      const expanded = (view.expanded_intent_steps || []).includes(step.id);
      return `
        <article class="nb-process-step ${isFocused ? "edit-focus" : ""}" id="nb-step-${html(step.id)}">
          <div class="nb-step-main">
            <div class="nb-step-index">
              ${index + 1}. ${html(step.name)}
              <span>${html(step.time)}</span>
            </div>
            <div>
              <div class="nb-step-summary">${html(step.summary)}</div>
              ${renderSectionTags(step.tags || [])}
            </div>
            <div class="nb-edit-tools">
              <button class="nb-soft-button" type="button" data-intent-step="${html(step.id)}">${expanded ? "收起说明" : "查看设计说明"}</button>
              <button class="nb-soft-button" type="button" data-edit-target="process:${html(step.id)}">编辑</button>
            </div>
          </div>
          ${expanded || isFocused ? renderIntentPanel(step) : ""}
    ...
    function renderEditPanel(view, title) {
      ...
      const patch1013F = renderPatchCandidate1013F(view);
      return `
        <div class="nb-edit-panel">
          <div class="nb-edit-panel-title">
            <span>小备建议 · ${html(title)}</span>
            <span class="quiet-tag">待你确认</span>
          </div>
          <div class="nb-right-note">${html(edit.judgment || "先把这段改成候选，不直接生效。")}</div>
          <div class="nb-step-candidate">${html(edit.candidate || "暂无候选。")}</div>
          <div class="nb-section-tags">
            ${(edit.impacts || []).map((item) => `<span class="nb-section-tag">${html(item)}</span>`).join("")}
          </div>
          <div class="nb-edit-tools">
            ${(edit.actions || []).map((action, index) => `
              <button class="nb-soft-button ${index === 0 ? "primary" : ""}" type="button" data-pending="小备已把「${html(action)}」放入待确认队列。">${html(action)}</button>
            `).join("")}
          </div>
          ${patch1013F}
        </div>
      ...
      return `
        <section class="nb-doc-section" id="nb-section-teaching-process">
          <div class="nb-doc-section-head">
            ...
            <span class="quiet-tag">每个环节可看设计说明</span>
          </div>
          <div class="nb-process-list">
    ...
    function renderReasoningBinding1013F(view) {
      ...
      if (!binding) return "";
      const activeEventId = binding.active_event_id || "";
      return `
        <section class="nb-binding-card" id="nb-1013f-binding" aria-label="小备课堂推演候选">
          <div class="nb-binding-head">
            <div class="nb-binding-title">
              小备课堂推演候选
              <span>${html(binding.status || "只读候选")}</span>
            </div>
            <span class="quiet-tag">等你确认</span>
          </div>
          <div class="nb-binding-quote">小备判断：${html(binding.judgment)}</div>
          <div class="nb-binding-grid">
            <div class="nb-binding-mini"><strong>本课学习问题</strong>${html(binding.learning_problem)}</div>
            <div class="nb-binding-mini"><strong>目标变化</strong>${html(binding.target_shift)}</div>
            <div class="nb-binding-mini"><strong>课堂路线</strong>${html(binding.route_summary)}</div>
            <div class="nb-binding-mini"><strong>时间与承接</strong>${html(binding.time_arrangement)} ${html(binding.next_lesson_connection)}</div>
          </div>
          <div class="nb-event-list">
            ${(binding.events || []).map((event) => `
              <article class="nb-event-card ${event.id === activeEventId ? "active" : ""}">
                <div class="nb-event-name">
                  ${html(event.label)}
                  <span>${html(event.minutes)}</span>
                </div>
                <div class="nb-event-copy">
                  <strong>${html(event.focus)}</strong>
                  <span>问题：${html(event.question)}</span>
                  <span>任务：${html(event.task)}</span>
                  <span>证据：${html(event.evidence)}</span>
                  <span>变化：${html(event.change)}</span>
                </div>
                <button class="nb-soft-button" type="button" data-edit-target="process:explore">编辑</button>
              </article>
            `).join("")}
          </div>
        </section>
    ...
    function renderReasoningTrace(view) {
      ...
      return `
        <section class="nb-trace-card" aria-label="小备正在同步思考">
          <div class="nb-trace-head">
            <div class="nb-trace-title">
              <strong>${isDone ? "小备已整理出候选" : "小备正在同步思考"}</strong>
    ...
    function bindEvents() {
      document.body.addEventListener("click", (event) => {
        ...
        if (event.target.closest("#chatVoiceBtn")) {
          addPendingChange("小备已打开语音意图入口预演，等待教师继续输入。", "小备意图入口");
          return;
        }
      ...
      byId("chatUploadInput").addEventListener("change", (event) => {
        ...
        addPendingChange(`小备已接收资料「${names}${more}」的本地挂载预演，未上传到服务器。`, "资料入口");
        event.target.value = "";
      });
    ...
    function submitChatMessage() {
      ...
      if (model.active_view === "prepNotebook") {
        ...
        addPendingChange(`小备已收到意图：「${text}」，正在生成影响范围预演。`, "小备意图入口");
      }
      input.value = "";
    }
