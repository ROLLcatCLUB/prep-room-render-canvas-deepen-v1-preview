...
```text
当前周：整行高亮 + 左侧箭头 + 当前周胶囊
两课时槽：底层保留 slot_index = 1 / 2，视觉上不再显示第1课时 / 第2课时标签卡
未来课：灰化但保持文字可读
状态颜色：正常 / 未来 / 活动 / 节假日 / 调课 / 补课顺延保持克制区分
图例：轻量横向图例，不占主舞台
摘要指标：跟在年级切换后，仅显示图标和值，标题进入 hover title
右侧建议：保持轻面板，只承载问题、建议和确认入口
移动端：保留横向展开与滚动，不用固定像素宽度压住中间看板
```
...
当前 `PREP_ROOM_RENDER_CANVAS_DEEPEN_V1` 已更新为 `周课表 / 班级排课 / 学期规划` 三个视图。
...
周课表 -> 替换原周课包，作为本周课前执行入口
```

这样三个视图的职责会更清楚：

```text
周课表：本周哪天哪节、哪个班、上什么、课前包是否备齐
