...
STAGE_ID = "1013I_R6N_R6_MATERIAL_PROMPT_FRONTLOADED_HORIZONTAL"
...
NEXT_STAGE = "USER_REVIEW_BIG_UNIT_MATERIAL_PROMPT"
STAGE_DIR_NAME = "1013I_R6N_R6_material_prompt_frontloaded_horizontal"
...
HTML_NAME = "prep_room_render_canvas_deepen_v1_R6N_R6_material_prompt_frontloaded_horizontal.html"
VALIDATOR_NAME = "validate_1013I_R6N_R6_material_prompt_frontloaded_horizontal.py"
...
def r6n_r4_css() -> str:
    ...
    /* 1013I_R6N_R6: frontloaded horizontal material prompt */
      ...
      return `
        <section class="nb-material-front-prompt" aria-label="资料补充提示">
          <div class="nb-material-front-copy">
            ...
            <p class="nb-material-front-note">缺教材目录、单元页或课时安排时，小教只能给临时预览，不能生成稳定的大单元设计。</p>
          </div>
    ...
    function renderBigUnitPrepSurface(view) {
      return `
        <section class="nb-workspace" aria-label="大单元设计">
          ...
          <article class="nb-unit-reading-doc" data-r6n-r6-material-frontloaded="true">
...
def build_html(output_root: Path) -> str:
    base = output_root / R6N_R5_DIR_NAME / "prep_room_render_canvas_deepen_v1_R6N_R5_big_unit_lesson_chain_and_single_lesson_inheritance.html"
    source = base.read_text(encoding="utf-8")
    source = source.replace("师维 · 备课室 | R6N_R5 课时任务链", "师维 · 备课室 | R6N_R6 资料前置提示")
    source = source.replace('data-r6n-r5-lesson-chain-page="true"', 'data-r6n-r6-material-frontloaded="true"')
...
def create_screenshots(stage_dir: Path, html_path: Path) -> dict[str, Any]:
    ...
    for viewport in [{"id": "desktop", "width": 1440, "height": 1100}, {"id": "mobile", "width": 390, "height": 1100}]:
        out = stage_dir / f"ui_smoke_screenshot_1013I_R6N_R6_{viewport['id']}.png"
        cmd = [
            str(browser),
            "--headless=new",
            "--disable-gpu",
...
def build_result(output_root: Path, html_path: Path, visual_smoke: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        ...
        "material_prompt_heavy_but_compact": "还缺教材材料" in html_text and "不能生成稳定的大单元设计" in html_text and "font-size: 13px" in html_text and "font-size: 12px" in html_text,
        ...
        "layout_style_inherits_r6n_r5": 'data-r6n-r6-material-frontloaded="true"' in html_text and "nb-light-tag" in html_text and "nb-section-help" in html_text,
    ...
    result["final_status"] = FINAL_STATUS if not failures else "FAIL_1013I_R6N_R6_MATERIAL_PROMPT_FRONTLOADED_HORIZONTAL"
    return result
...
LAYOUT_STYLE_INHERITS_R6N_R5={str(result["layout_style_inherits_r6n_r5"]).lower()}
...
{STAGE_DIR_NAME}/1013I_R6N_R6_result.json
{STAGE_DIR_NAME}/1013I_R6N_R6_report.md
scripts/{VALIDATOR_NAME}
source_delta_1013I_R6N_R6/scripts/{VALIDATOR_NAME}
{RESEARCH_NAME}
```
""")
    write_text(stage_dir / "1013I_R6N_R6_report.md", f"""# 1013I_R6N_R6 Report

This patch moves material supplementation to the top of the big-unit reading page.
...
def validate_result(result: dict[str, Any]) -> None:
    if result.get("failed_checks"):
        raise SystemExit("R6N_R6 validation failed: " + ", ".join(result["failed_checks"]))
...
def main() -> None:
    ...
    write_json(stage_dir / "visual_smoke_1013I_R6N_R6.json", visual_smoke)
    result = build_result(output_root, html_path, visual_smoke)
    result["secret_scan_hits"] = scan_secrets([html_path])
    write_json(stage_dir / "1013I_R6N_R6_result.json", result)
    write_json(stage_dir / "material_prompt_manifest_1013I_R6N_R6.json", {"stage": STAGE_ID, "inherits_from": INHERITS_FROM, "research_source": RESEARCH_NAME, "boundary": boundary()})
    write_docs(output_root, stage_dir, result)
    source_delta = output_root / "source_delta_1013I_R6N_R6" / "scripts"
    ...
    print("ALL_1013I_R6N_R6_MATERIAL_PROMPT_FRONTLOADED_CHECKS_OK")
